...
@Component({
  selector: 'app-candidati-da-verificare',
  templateUrl: './candidati-da-verificare.component.html',
  styleUrls: ['./candidati-da-verificare.component.scss']
})
export class CandidatiDaVerificareComponent implements OnInit {
    ...
    constructor(private db: AngularFireDatabase, private f: FirebaseApp) {
        ...
        */
        this.db.list('/candidature/candidature_da_verificare').snapshotChanges().map(actions => {
            return actions.map(action => ({ key: action.key, ...action.payload.val() }));
        }).subscribe(items => {
            items.forEach( it => {
